...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2>Tambah Lauk / Ekstra</h2>
        <a href="{{ route('admin.lauks.index') }}" class="btn btn-outline-secondary">Kembali</a>
    </div>

    <div class="card shadow-sm">
        <div class="card-body">
            <form action="{{ route('admin.lauks.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label">Nama Lauk / Ekstra <span class="text-danger">*</span></label>
                        <input type="text" name="nama_lauk" class="form-control" value="{{ old('nama_lauk') }}" placeholder="Contoh: Ayam suwir / Telur asin" required>
                        @error('nama_lauk')
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="tipe" id="tipeSelect" class="form-select" required>
                            ...
                            <option value="utama" @selected(old('tipe') === 'utama')>Lauk Utama</option>
                            <option value="tambahan" @selected(old('tipe') === 'tambahan')>Ekstra Lauk (Tambahan)</option>
                        </select>
                        @error('tipe')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" name="harga" class="form-control" value="{{ old('harga') }}" min="0" placeholder="Contoh: 12000 / 3000" required>
                        </div>
                        @error('harga')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        @error('gambar')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                        <small class="text-muted mt-1 d-block">Pilih file gambar jika ingin menampilkan foto lauk (format: JPG, JPEG, PNG, maks 2MB).</small>
                    </div>

                    <div class="col-md-6" id="premiumContainer">
                        <label class="form-label d-block">Pengaturan Lauk Utama</label>
                        <div class="form-check form-switch pt-2">
                            <input class="form-check-input" type="checkbox" name="is_premium" id="is_premium" value="1" @checked(old('is_premium'))>
                ...
                <hr class="my-4">
                <button class="btn btn-primary" type="submit">Tambah Lauk</button>
            </form>
        </div>
    </div>
